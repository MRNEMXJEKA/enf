...
class UpdateCartItemViews(CartMixin, View):
    @transaction.atomic
    def post(self, request, item_id):
        cart = self.get_cart(request)
        cart_item = get_object_or_404(CartItem, id=item_id, cart=cart)
        
        quantity = int(request.POST.get('quanttity', 1))
...
class RemoveCartItemViews(CartMixin, View):
    def post(self, request, item_id):
        cart = self.get_cart(request)
...
class CartCountViews(CartMixin, View):
    def get(self, request):
        cart = self.get_cart(request)
        return JsonResponse({
            'total_items': cart.total_items,
